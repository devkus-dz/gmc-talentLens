...
export interface Certification { name: string; issuer: string; date?: string | null; }
...
export default function CertificationsCard({ certifications, onAdd, onRemove, onEdit }: Props) {
    return (
        <div>
            ...
            <div className="flex flex-col gap-3">
                {certifications.length > 0 ? certifications.map((cert, index) => (
                    <div
                        key={index}
                        className="bg-base-200/50 p-4 rounded-2xl border border-base-content/5 flex gap-3 items-center group relative pr-10 cursor-pointer hover:border-primary/20 transition-colors"

// frontend/src/app/admin/candidates/[id]/page.tsx
import React, { JSX } from 'react';
import { cookies } from 'next/headers';
import Link from 'next/link';
import { ArrowLeft, Mail } from 'lucide-react';

// Reuse your existing profile components
...
/**
 * Server Component to display a Candidate's full read-only profile.
 * @async
 * @param {CandidateProfilePageProps} props 
 * @returns {Promise<JSX.Element>}
 */
export default async function AdminCandidateProfilePage({ params }: CandidateProfilePageProps): Promise<JSX.Element> {
    ...
    // Fetch user and resume data concurrently
    const [userRes, resumeRes] = await Promise.all([
        fetch(`${backendUrl}/users/${candidateId}`, { headers, cache: 'no-store' }),
        fetch(`${backendUrl}/resumes/user/${candidateId}`, { headers, cache: 'no-store' })
    ]);

    // FIX: The backend wraps the user inside a `data` property
    const userResponse = userRes.ok ? await userRes.json() : null;
    const user = userResponse?.data || null;

    // The resume endpoint does not wrap in `data` (returns the object directly)
    ...
    if (!user) {
        return (
            <div className="p-8 text-center mt-20">
                ...
                <Link href="/admin/candidates" className="btn btn-primary mt-6">Back to Directory</Link>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 animate-fade-in pb-20">
            ...
            <div className="print:hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <Link href="/admin/candidates" className="btn btn-sm btn-ghost gap-2 hover:bg-base-200">
